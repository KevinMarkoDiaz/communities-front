...
export default function ScrollCarousel({ children }) {
  ...
  return (
    <div className="relative w-full overflow-x-hidden gap-32">
      <button
        onClick={() => scroll("left")}
        className=" md:flex absolute left-2 text-[#Fff] md:text-black top-1/2 -translate-y-2/2  md:-translate-y-2/2 z-20 md:bg-white p-2 rounded-full md:shadow"
      >
        <FiChevronLeft size={20} />
      </button>
      <button
        onClick={() => scroll("right")}
        className=" md:flex absolute right-2 text-[#fff] md:text-black top-1/2 -translate-y-2/2 z-20 md:bg-white p-2 rounded-full md:shadow"
      >
