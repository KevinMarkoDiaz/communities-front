...
export default function Resumen({ resumen = [], user }) {
  ...
  // Si querés ocultar categorías si no es admin
  ...
  return (
    <div className="bg-[#F7F7F7] px-2 py-3 md:p-3 lg:p-6 rounded-2xl h-full">
      ...
      <div className="grid grid-cols-1 md:grid-cols-1 lg:grid-cols-1 xl:grid-cols-2 gap-3">
        {filteredResumen.map(({ label, value, path, requireAdmin }, index) => {
          const Icon = iconMap[label.replace("Tus ", "")] || FiFolder;
          ...
          return (
            ...
            >
              <div className="w-7 h-7 flex items-center justify-center rounded-sm text-orange-500">
